...
public class AccountServiceImpl implements AccountService {
    ...
    public void changeName(Integer accountid, String name) {
            ...
            () -> new RuntimeException("Id da conta não encontrada")
        );
        if (name == null || name.trim().isEmpty()) throw new RuntimeException("O nome não pode ser vazio!");
    ...
    public void deposit(Integer accountId, BigDecimal value) {
        if (value == null || value.compareTo(BigDecimal.ZERO) <= 0) throw new RuntimeException("O valor do depósito deve ser maior que zero.");
        AbstractAccount account = repoAccount.findById(accountId).orElseThrow(
    ...
    public void withdraw(Integer accountId, BigDecimal value) {
        if (value == null || value.compareTo(BigDecimal.ZERO) <= 0) throw new RuntimeException("O valor do saque deve ser maior que zero.");
        AbstractAccount account = repoAccount.findById(accountId).orElseThrow(
            () -> new RuntimeException("Conta não encontrada para o ID informado.")
        );
    ...
    public void remove(Integer accountId) {
        if (accountId == null || accountId < 0) throw new RuntimeException("Conta não encontrada para o ID informado.");
