...
public class CategoryServiceImpl implements CategoryService{
    ...
    public void create(String name, CategoryType type) {
        ...
        for (Category c : categories) {
            if (c.getName().equals(name)) {
                throw new RuntimeException("Já existe esse nome na lista de catogorias.");
            }
        }
    ...
    public void remove(Integer categoryId) {
        if (categoryId == null || categoryId < 0) throw new RuntimeException("Categoria não encontrada para o ID informado.");
