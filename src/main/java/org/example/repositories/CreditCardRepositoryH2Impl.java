...
public class CreditCardRepositoryH2Impl implements CreditCardRepository {
  ...
  public void save(CreditCard creditCard) {
    String sql =
      "INSERT INTO cartaocredito (nome, limite, saldo, dia_fechamento, dia_vencimento, conta_id) VALUES (?, ?, ?, ?, ?, ?)";
    try (
  ...
  public Optional<CreditCard> findById(Integer id) {
    ...
    try (
      ...
      try (ResultSet rs = stmt.executeQuery()) {
        if (rs.next()) {
          AccountBank bank;
          AbstractAccount account = this.repoAccount.findById(
            rs.getInt("conta_id")
          ...
          if (account instanceof AccountBank) {
            ...
            throw new RuntimeException(
              "O ID informado não pertence a um Banco!"
            );
          }
  ...
  public void update(CreditCard creditCard) {
    String sql =
      "UPDATE cartaocredito SET nome = ?, limite = ?, saldo = ?, dia_fechamento = ?, dia_vencimento = ?, conta_id = ? WHERE id = ?";
