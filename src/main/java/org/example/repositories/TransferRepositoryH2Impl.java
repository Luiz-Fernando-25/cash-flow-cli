...
public class TransferRepositoryH2Impl implements TransferRepository{
    ...
    public void save(Transfer transfer) {
        String sql = "INSERT INTO movimentacao (saida_id, entrada_id) VALUES (?, ?)";
        ...
        PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    transaction = this.repoTransaction.findById(rs.getInt("saida_id")).orElseThrow(
                    ...
                    if (transaction instanceof TransactionOutput) {
                        ...
                        throw new RuntimeException("O ID informado não pertence a uma transação de saida!");
                    }

                    transaction = this.repoTransaction.findById(rs.getInt("entrada_id")).orElseThrow(
                    ...
                    if (transaction instanceof TransactionInput) {
                        ...
                        throw new RuntimeException("O ID informado não pertence a uma transação de entrada!");
                    }
                    
                    Transfer transfer = new Transfer(id, transactionOutput, transactionInput);
        ...
        ResultSet rs = stmt.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                if (transactionInMemory.containsKey(transactionInputId)){
                    if (transactionInMemory instanceof TransactionInput){
                        transactionInput = (TransactionInput) transactionInMemory.get(transactionInputId);
                    } else {
                        throw new RuntimeException("O ID informado não pertence a uma transação de entrada!");
                    }
                } else {
                    AbstractTransaction transaction = this.repoTransaction.findById(transactionInputId).orElseThrow(
                        () -> new RuntimeException("Transação não encontada")
                    );
                    if (transaction instanceof TransactionInput){
                        transactionInput = (TransactionInput) transaction;
                ...
                if (transactionInMemory.containsKey(transactionOutputId)){
                    if (transactionInMemory instanceof TransactionOutput){
                        transactionOutput = (TransactionOutput) transactionInMemory.get(transactionOutputId);
                    } else {
                        throw new RuntimeException("O ID informado não pertence a uma transação de saida!");
                    }
                } else {
                    AbstractTransaction transaction = this.repoTransaction.findById(transactionOutputId).orElseThrow(
                        () -> new RuntimeException("Transação não encontada")
                    );
                    if (transaction instanceof TransactionOutput){
                        transactionOutput = (TransactionOutput) transaction;
                ...
                transfer.add(new Transfer(rs.getInt("id"), transactionOutput, transactionInput));
            }
            return transfer;
    ...
    public void update(Transfer transfer) {
        String sql = "UPDATE movimentacao SET saida_id = ?, entrada_id WHERE id = ?";
